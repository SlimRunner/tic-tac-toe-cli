...
// Sets the value of an individual cell in the board and returns the state of
// the board
t3g::T3_board_state t3g::T3_Board::set_cell_state(cell_loc x_loc,
                                                  cell_loc y_loc,
                                                  T3_cell_state state) {
  cell_loc index;
...
t3g::T3_board_state t3g::T3_Board::set_cell_state(cell_loc serial,
                                                  T3_cell_state state) {
...
t3g::T3_board_state t3g::T3_Board::set_cell_state(T3_cell_state *in_board,
                                                  size_t arr_size) {
...
t3g::T3_cell_state t3g::T3_Board::get_cell_state(cell_loc x_loc,
                                                 cell_loc y_loc) const {
  if (x_loc > BOARD_SIDES || y_loc > BOARD_SIDES) {
    throw Ex_out_of_bounds();
  }
...
t3g::T3_cell_condition t3g::T3_Board::get_cell_condition(cell_loc x_loc,
                                                         cell_loc y_loc) const {
  if (x_loc > BOARD_SIDES || y_loc > BOARD_SIDES) {
    throw Ex_out_of_bounds();
  }
...
t3g::T3_cell_condition
t3g::T3_Board::get_cell_condition(cell_loc serial) const {
...
t3g::T3_board_state t3g::T3_Board::determine_board_state() {
  enum Normal_states { EMPTY_BOARD = 0, FULL_BOARD = 0x1ff };
...
// Determines the state of the board, updates the condition array accordingly,
// and sets the last changed item
t3g::T3_board_state t3g::T3_Board::determine_board_state(cell_loc x_loc,
                                                         cell_loc y_loc) {
  ...
  cell_loc index = get_xy_index(x_loc, y_loc, BOARD_SIDES);
  m_cond_board[index] =
      m_cond_board[index] | t3g::T3_cell_condition::LAST_MOVE; // sets last move
...
bool t3g::T3_Board::is_win_state(size_t bit_field) {
  ...
  using cell_condition = t3g::T3_cell_condition;

  const size_t ARR_LEN = 8;
  bool retVal = false;
  Win_states ws_array[] = {HORZ_UPP, HORZ_MID, HORZ_LOW, VERT_UPP,
                           VERT_MID, VERT_LOW, DIAG_0T8, DIAG_2T6};

  for (size_t i = 0; i < ARR_LEN; ++i) {
    if WIN_MASK (bit_field, static_cast<size_t>(ws_array[i])) {
      switch (ws_array[i]) {
      case HORZ_UPP:
        m_cond_board[0] = m_cond_board[1] = m_cond_board[2] =
            cell_condition::WIN_FLAG;
        break;
      case HORZ_MID:
        m_cond_board[3] = m_cond_board[4] = m_cond_board[5] =
            cell_condition::WIN_FLAG;
        break;
      case HORZ_LOW:
        m_cond_board[6] = m_cond_board[7] = m_cond_board[8] =
            cell_condition::WIN_FLAG;
        break;
      case VERT_UPP:
        m_cond_board[0] = m_cond_board[3] = m_cond_board[6] =
            cell_condition::WIN_FLAG;
        break;
      case VERT_MID:
        m_cond_board[1] = m_cond_board[4] = m_cond_board[7] =
            cell_condition::WIN_FLAG;
        break;
      case VERT_LOW:
        m_cond_board[2] = m_cond_board[5] = m_cond_board[8] =
            cell_condition::WIN_FLAG;
        break;
      case DIAG_0T8:
        m_cond_board[0] = m_cond_board[4] = m_cond_board[8] =
            cell_condition::WIN_FLAG;
        break;
      case DIAG_2T6:
        m_cond_board[2] = m_cond_board[4] = m_cond_board[6] =
            cell_condition::WIN_FLAG;
        break;
      default:
        break;
