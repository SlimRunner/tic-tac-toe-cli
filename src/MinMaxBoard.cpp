...
void mmx::MinMaxBoard::calc_board(const t3g::T3_Match &match) {
  // calculates the minimax for the passed match and assigns the result to
  // itself
  *this = get_choice_map(match);
}

t3g::cell_loc mmx::MinMaxBoard::rand_cell_query(Mmb_States state,
                                                Rank_Range rank) {
  ...
  states_vector cell_states{Mmb_States::OCCUPIED, Mmb_States::NEUTRAL,
                            Mmb_States::TIE_MOVE, Mmb_States::LOSE_MOVE,
                            Mmb_States::WIN_MOVE};
  ...
  for (states_vector::iterator it = cell_states.begin();
       it != cell_states.end(); ++it) {
  ...
  std::sort(score_subset.begin(), score_subset.end(),
            [](const MinMaxScore &lhs, const MinMaxScore &rhs) {
  ...
  for (cell_vector::iterator it = cell_collector.begin();
       it != cell_collector.end(); ++it) {
    ...
    if (static_cast<bool>(rank & (Rank_Range::ANY_RANK)) && max == min) {
      cell_range.push_back(*it);
    }
  }
...
inline mmx::MinMaxScore max_score(const mmx::MinMaxScore &bestVal,
                                  const mmx::MinMaxScore &newVal) {
  if (bestVal > newVal)
    return bestVal;
  else
    return newVal;
...
inline mmx::MinMaxScore min_score(const mmx::MinMaxScore &bestVal,
                                  const mmx::MinMaxScore &newVal) {
  if (bestVal < newVal)
    return bestVal;
  else
...
mmx::MinMaxScore t3_minimax(t3g::T3_Match match, t3g::cell_loc seed_loc,
  ...
  case t3g::T3_board_state::O_WIN_BOARD:
    if (!isMax)
      return mmx::MinMaxScore(MAX_SCORE - depth, mmx::Mmb_States::LOSE_MOVE);
    else
      return mmx::MinMaxScore(-MAX_SCORE + depth, mmx::Mmb_States::WIN_MOVE);
    break;
  default:
    break;
  }
  ...
  mmx::MinMaxScore rec_score;  // Intermediary variable that receives the return
                               // of the recursion
  // generates the Minimax first assumption (worse case scenario)
  best_score = mmx::MinMaxScore(SCORE_BOUND * (isMax ? -1 : 1),
