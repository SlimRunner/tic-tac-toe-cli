...
tic::TicTacToe_cli::TicTacToe_cli()
    : m_thisMatch(t3g::T3_cell_state::X_STATE), m_ai_diff(AI_Level::AI_MEDIUM) {
  m_starting_symbol = m_thisMatch.get_curr_symbol();
}

tic::TicTacToe_cli::TicTacToe_cli(t3g::T3_cell_state starting_symbol)
    : m_thisMatch(starting_symbol), m_ai_diff(AI_Level::AI_MEDIUM) {
  m_starting_symbol = m_thisMatch.get_curr_symbol();
}

tic::TicTacToe_cli::TicTacToe_cli(AI_Level ai_difficulty)
    : m_thisMatch(t3g::T3_cell_state::X_STATE), m_ai_diff(ai_difficulty) {
  m_starting_symbol = m_thisMatch.get_curr_symbol();
}

tic::TicTacToe_cli::TicTacToe_cli(AI_Level ai_difficulty,
                                  t3g::T3_cell_state starting_symbol)
    : m_thisMatch(starting_symbol), m_ai_diff(ai_difficulty) {
  m_starting_symbol = m_thisMatch.get_curr_symbol();
}
...
void tic::TicTacToe_cli::runGame() {
  ...
  do {
  } while (runMainMenu() != OptMainMenu::MNU_EXIT);
}
...
tic::OptMainMenu tic::TicTacToe_cli::runMainMenu() {
  ...
  std::cout << "\n" << ASTERISK_SEPARATOR;
  std::cout << "\nMAIN MENU\n\n"; // cascade pass the menu name as argument to
                                  // the next menu
  ...
  userSel =
      static_cast<OptMainMenu>(prompt_getChar("\nSelect an option: ", true));

  switch (userSel) {
  case tic::OptMainMenu::MNU_PLAY:
    do {
    } while (runPlayMenu() != OptPlayMatch::SUB_EXIT);
    break;
  case tic::OptMainMenu::MNU_SETTINGS:
    do {
    } while (runSettings() != OptSettings::SUB_EXIT);
    break;
...
tic::OptPlayMatch tic::TicTacToe_cli::runPlayMenu() {
  ...
  std::cout << "\n" << ASTERISK_SEPARATOR;
  std::cout << "\nMAIN MENU > PLAY MATCH\n\n"; // cascade pass the menu name as
                                               // argument to the next menu
  ...
  ")\n"; std::cout << "Rev Match vs AI\t(" <<
  ENUM_TO_INT(OptPlayMatch::SUB_REV_AI) << ")\n";*/
  std::cout << "Go back\t\t(" << ENUM_TO_INT(OptPlayMatch::SUB_EXIT) << ")\n";
  userSel =
      static_cast<OptPlayMatch>(prompt_getChar("\nSelect an option: ", true));
...
tic::OptSettings tic::TicTacToe_cli::runSettings() {
  ...
  std::cout << "Toggle symbol\t(" << ENUM_TO_INT(OptSettings::SUB_DEF_SYMBOL)
            << ") -> " << getSymbol(m_starting_symbol) << " moves first\n";
  std::cout << "AI difficulty\t(" << ENUM_TO_INT(OptSettings::SUB_AI_DIFFICULTY)
            << ") -> " << getDiffName(m_ai_diff) << "\n";
  std::cout << "Go back\t\t(" << ENUM_TO_INT(OptSettings::SUB_EXIT) << ")\n";
  userSel =
      static_cast<OptSettings>(prompt_getChar("\nSelect an option: ", true));
  ...
  do {
    ...
    try {
      if (!ai_match || userSymbol == m_thisMatch.get_curr_symbol()) {
        ...
        while (m_thisMatch.get_cell_condition(
                   serialLoc = getSerialFromKeypad(symLoc)) !=
               t3g::T3_cell_condition::NORMAL) {
...
t3g::cell_loc tic::TicTacToe_cli::getSerialFromKeypad(char inCharacter) const {
  switch (inCharacter) {
  case '7':
    return 0U; // upper left
  case '8':
    return 1U; // upper middle
  case '9':
    return 2U; // upper right
  case '4':
    return 3U; // mid left
  case '5':
    return 4U; // mid middle
  case '6':
    return 5U; // mid right
  case '1':
    return 6U; // lower left
  case '2':
    return 7U; // lower middle
  case '3':
    return 8U; // lower right
  default:
    return UINTMAX_MAX; // error
  }
}
...
static char prompt_getChar(const char *message, bool makeUpper) {
  ...
  while ((std::cout
          << message) // return is irrelevant, it's here to loop the message
         && (!std::getline(std::cin,
                           inStr) // return is null when input is not valid
             || (inStr.length() != 1))) {
    std::cout << "Not a valid character input." << std::endl;
    if (std::cin.eof())
      std::cin.clear(); // clears error if user enters ^Z(EOF)
  }
