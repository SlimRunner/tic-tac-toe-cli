...
struct MinMaxScore {
  ...
  MinMaxScore() : score(0), state(Mmb_States::NEUTRAL) {}
...
public:
  ...
  class Proxy {
  public:
    Proxy(MinMaxBoard &ref, t3g::cell_loc index) : m_ref(ref), m_index(index) {}
...
private:
  // SUGGESTION: use vector instead since it's already being included in the
  // .cpp
  MinMaxScore m_arr[t3g::BOARD_SIZE]{}; // static array initialized to default
                                        // value (Mbm_States::NEUTRAL)
};
} // namespace mmx
#endif // !MINMAXBOARD_H
