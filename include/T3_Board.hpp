...
enum class T3_cell_state { NULL_STATE = 0, X_STATE = 1, O_STATE = 2 };

enum class T3_cell_condition { NORMAL = 0, LAST_MOVE = 1, WIN_FLAG = 2 };
inline T3_cell_condition operator|(T3_cell_condition lhs,
                                   const T3_cell_condition &rhs) {
  return static_cast<T3_cell_condition>(static_cast<int>(lhs) |
                                        static_cast<int>(rhs));
}
...
protected:
  T3_board_state set_cell_state(cell_loc x_loc, cell_loc y_loc,
                                T3_cell_state state);
  T3_board_state set_cell_state(cell_loc serial, T3_cell_state state);

  // This function is intended to be used to load a saved game, should not be
  // used as a replacement of the other overload since this one bypasses
  // validation
